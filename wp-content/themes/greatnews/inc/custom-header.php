 ...
 *
 * @uses great_news_header_style()
 */
function great_news_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'great_news_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'great_news_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'greatnews' ),
		),
	) );
}
add_action( 'after_setup_theme', 'great_news_custom_header_setup' );

if ( ! function_exists( 'great_news_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see great_news_custom_header_setup().
	 */
	function great_news_header_style() {
		...
		wp_add_inline_style( 'great-news-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'great_news_header_style', 10 );