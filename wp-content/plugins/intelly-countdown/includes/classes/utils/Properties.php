...
class ICP_Properties {
	...
	private function encode( $t, $v ) {
		...
		switch ( strtolower( $t ) ) {
			...
			case 'f':
				...
				break;
			case 'd':
				$v = $icp->Utils->formatDatetime( $v );
				break;
			case 'b':
				$v = ( $icp->Utils->isTrue( $v ) ? 'true' : 'false' );
				break;
			case 'a':
	...
	public function getFile( $key, $default = false ) {
		...
		if ( is_dir( $file ) ) {
			...
			$file = str_replace( '/', DIRECTORY_SEPARATOR, $file );
			if ( ! $icp->Utils->endsWith( $file, DIRECTORY_SEPARATOR ) ) {
