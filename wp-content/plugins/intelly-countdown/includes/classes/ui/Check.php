...
class ICP_Check {
	...
	public function __construct() {
		$utils      = new ICP_Utils();
		$this->data = $utils->sanitize_post_or_get( array_merge( wp_unslash( $_POST ), wp_unslash( $_GET ) ) );
	}
	...
	private function error( $name ) {
		...
		$result = false;
		$k      = $icp->Ui->CF->prefix . '.' . $name . '.check';
		$v      = $icp->Lang->L( $k );
		if ( $v != $k ) {
			//this is a mandatory field so we give error
			$icp->Options->pushErrorMessage( $v );
			$result = true;
		}
		return $result;
	}
	...
	public function hasErrors() {
		global $icp;
		return $icp->Options->hasErrorMessages();
	}
}
